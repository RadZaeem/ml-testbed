        ...
        with bit_utils.replace_variable(replace_w):
            with tf.variable_scope(scope or type(self).__name__):
                with tf.variable_scope("Gates"):
                    ...
                    #     #from tensorflow.python.ops import rnn_cell_impl
                    #     rnn_cell_impl._linear([inputs, state],
                    #                            2 * self._num_units,1.0,True))#, True))#, 1.0))
                    r, u = tf.split(
                        
                        #tf.nn.rnn_cell._linear
                        #from tensorflow.python.ops import rnn_cell_impl
                        rnn_cell_impl._linear([inputs, state],
                                               2 * self._num_units,1.0), 2, 1)#, True))#, 1.0))
                    r, u = tf.sigmoid(r), tf.sigmoid(u)
                with tf.variable_scope("Candidate"):
                    c = self._activation(rnn_cell_impl._linear([
                            inputs, bit_utils.round_bit(r * state,
                                                        bit=self._f_bit)],
                            self._num_units,0))#, True))
