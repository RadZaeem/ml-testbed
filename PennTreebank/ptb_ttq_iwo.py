...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        def get_basic_cell():
            ...
            # cell = ttq_rnn.TtqGRUCell(num_units=conf.hidden_size,thre=0.05,#)
    ...
    # with bit_utils.replace_variable(replace_w):
        ...
        #     print ("OP under a")
        #     print(x.op.name)
        #     return tw_ternarize_input(x, 0.05) 
        # with bit_utils.replace_variable(a):
        ...
        input_feature = tw_ternarize_input(input_feature, 0.05)
        ...
        # output = tw_ternarize_output(output,0.05)
        ...
        # print("LOGIT", repr(logits.variables))
        ...
        # weights = tf.get_variable("fc/kernel")
  #       weights = tf.get_default_graph().get_tensor_by_name(
  # os.path.split(logits.name)[0] + '/kernel:0')
  #       tf.summary.histogram(logits.name, weights)


        # with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
        # with varreplace.remap_variables(lambda x:tw_ternarize_output(x,0.05)):
        #     softmax_w = tf.get_variable("softmax_w", [conf.hidden_size, conf.vocab_size])
        #     # softmax_b = tf.get_variable("softmax_b", [conf.vocab_size])
        #     print("SOFTMAX")
        #     print(softmax_w)
        # logits = tf.matmul(output, softmax_w) + softmax_b
        # loss = tf.contrib.legacy_seq2seq.sequence_loss_by_example(
        #         [logits],
        #         [tf.reshape(self.nextinput, [-1])],
        #         [tf.ones([conf.batch_size * conf.num_steps])])
        # with tf.control_dependencies(update_state_ops):
        #     self.cost = tf.truediv(tf.reduce_sum(loss),
        #                            tf.cast(conf.batch_size, tf.float32), name='cost')  # log-perplexity
